...
if [ -n "$RUNNING_PIDS" ]; then
  fail "다른 쌀먹이 실행 중입니다. 메뉴의 ‘종료’로 끝낸 뒤 검사할 앱만 다시 지정하세요. PID: $(echo "$RUNNING_PIDS" | tr '\n' ' ')"
fi
...
for _ in $(seq 1 20); do
  TRAY_RECORD="$(osascript -e 'tell application "System Events"' -e 'tell process "ssalmeok"' -e 'set trayItems to every menu bar item of menu bar 2 whose description is "status menu"' -e 'if (count trayItems) is not 1 then return ""' -e 'set trayItem to item 1 of trayItems' -e 'set {itemX, itemY} to position of trayItem' -e 'set {itemWidth, itemHeight} to size of trayItem' -e 'return (name of trayItem as text) & "|" & itemX & "|" & itemY & "|" & itemWidth & "|" & itemHeight' -e 'end tell' -e 'end tell' 2>/dev/null || true)"
...
if [ "$PIXEL_CONFIRMATION" != "y" ] && [ "$PIXEL_CONFIRMATION" != "Y" ]; then
  fail "사람이 확인하는 픽셀 관문을 통과하지 못했습니다."
fi
